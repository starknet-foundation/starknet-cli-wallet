...
export function prettyPrintFee(fee: EstimateFee) {
  console.log(`Overall Fee: ${ethers.utils.formatEther(fee.overall_fee.toString())}`);
  if (fee.gas_consumed !== undefined) {
    console.log(`Gas Consumed: ${fee.gas_consumed.toString()}`);
  }
  if (fee.gas_price !== undefined) {
    console.log(`Gas Price: ${ethers.utils.formatUnits(fee.gas_price.toString(), 9)}`);
  }
  if (fee.suggestedMaxFee !== undefined) {
    console.log(`Suggested Max Fee: ${ethers.utils.formatEther(fee.suggestedMaxFee.toString())}`);
  }
}
