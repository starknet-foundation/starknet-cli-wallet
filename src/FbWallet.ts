import fs from "fs";
import { prettyPrintFee, abort } from "./util";
import {
  Contract,
  json,
  Account,
...
export class FbWallet {
  public account: Account;
  private signer: FbSigner;
  ...
  constructor(
    ...
    let fbSigner = new FbSigner(apiSecret, apiKey, uncompressedPubKey, vaultAccountId, assetId, bip44addressIndex);
    this.signer = fbSigner;
    this.account = new Account(provider, address, fbSigner);
    return;
  }
  ...
  async getBalance(tokenAddress?: string) {
    return FbWallet.getBalance(this.account.address, this.account, tokenAddress);
  }
  ...
  async deployAccount(): Promise<Account> {
    ...
    let pubKeyTrimmed = publicKey.substring(4);
    ...
    console.log("Future contract address", contractETHAccountAddress);
